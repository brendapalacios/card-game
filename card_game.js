...
function shuffle (deck){
  ...
  while (currentIndex !== 0){
    ...
    temporaryValue = shuffledDeck[currentIndex] /* taking a random card from the deck and assigning it to the temporary value variable*/
    // currentIndex = currentIndex -1     this is the same as below
shuffledDeck[currentIndex] = shuffledDeck[randomIndex] /* randomIndex card is going to replace currentIndex card */
shuffledDeck[randomIndex] = temporaryValue /* currentIndex card is going to replace the randomIndex card */
    currentIndex--
...
// 5. Done.
function greet() {
  const name = getInput('Welcome to the card game!' + ' What is your name?') 
console.log(name) 
return name //has to be within curly brace because it's part of the function
}
...
function guess (card1, card2) {
  console.log(card1.rank, card1.suit) //make sure it matches key part in the object, not the arrays

  let input = getInput("Do you think the next card will be 'h' or 'l' than your current card?")
...
else { console.log("Guess 'h' or 'l'. You get no points for this round.")
return false 
}
...
function playGame() {
  const deck = shuffle(buildDeck()) //this is a function call. It's a functional call within a function call.

  const playerName = greet()
  let score = 0
let currentCard = deck.pop() //assign is the same as equals  //deck.pop() <- this is a method call which is why we have open/closed parentheses
while (score < 5 && score < deck.length){
 let nextCard = deck.pop()  //use let, const you won't be able to reassign later
if(guess(currentCard, nextCard)) {
 score++
 console.log(`Congratulations! Your score is ${score}`) 
}
else{
  console.log("You are wrong. You recieved no points.")
}
currentCard = nextCard
}
deck.length == 0 ? console.log('You are out of card. You lost.'): console.log('You won') //this means we're at the end of the deck, we're checking the deck array has reached 0.
